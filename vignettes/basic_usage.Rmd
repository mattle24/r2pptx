...
`rpptx` leverages the power of the `officer` package and the ease of use of powerpoint
...
template. This is a normal `.pptx` file that has the template we want in `Slide Master`. By default, `rpptx` uses the default template in `officer`, which seems 
